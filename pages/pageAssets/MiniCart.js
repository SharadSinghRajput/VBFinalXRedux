...
export default function MiniCart({Design}) {
  ...
  return (
    <>
    {SessionAction ?
      <div className={`fixed bottom-12 right-36 z-10 ${Design === "cart" || Design === "place-order" || Design === "order-details" ? "w-[0] overflow-hidden" : "w-[500px] max-md:w-16 "}`}>
        <ul role="list" className="grid grid-cols-1 gap-x-6 gap-y-8 xl:gap-x-8 shadow-xl">
          {products ? <>
            {products[products.length - 1] ? <>
              <li className="overflow-hidden rounded-xl border border-orange-200">
                ...
                <div className="flex items-center justify-between gap-x-4 border-b border-gray-900/5 bg-white p-4">
                  <button onClick={()=> router.push('cart')} className="w-10 h-10 bg-orange-100/50 rounded-full flex justify-center items-center">
                  ...
                  <span className="absolute top-2 right-2 md:hidden">{products.length}</span>
                  ...
                  type="button" className="rounded-md max-md:hidden bg-orange-500 flex justify-center items-center gap-3 w-24 px-3.5 py-1.5 text-sm font-normal text-white ">
